	...
	return () => {
		...
		browser.on('serviceUp', function(service) {
			if (service.txtRecord && service.txtRecord.fn === configuration.device_name) {
				...
				console.log('Unable to find a chrome cast for name ',configuration.device_name, ' for service ',service.txtRecord)
			}
		});
		browser.start();
	}
...
function loadTheMedia(host, configuration) {
	connectToTheChromecast(host, function(client) {
		...
		client.launch(DefaultMediaReceiver, function(err, player) {
			player.load(configuration.media, {
				autoplay: true
			}, function(err, status) {
